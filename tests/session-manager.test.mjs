...
import { createMockClient, createTempDir, cleanupTempDir } from './helpers.mjs';
...
describe('SessionManager', () => {
  ...
  it('updateSession rejects invalid transitions', async () => {
    ...
    await assert.rejects(
      () => manager.updateSession(job.jobId, { status: 'running' }),
  ...
  it('updateSession throws for missing job', async () => {
    await assert.rejects(
      () => manager.updateSession('no-such-job', { status: 'running' }),
