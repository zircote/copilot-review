...
describe('validateReview', () => {
  ...
  const validReviewWithFindings = {
    ...
    findings: [
      { file: 'a.js', severity: 'critical', message: 'Bug here.' },
    ],
  };
  ...
  it('rejects findings that is not an array', () => {
    const { valid, errors } = validateReview({ summary: 'Ok.', verdict: 'approve', findings: 'none' });
    assert.equal(valid, false);
...
describe('buildProseFallback', () => {
  it('creates synthetic ReviewResult from prose', () => {
    ...
    assert.equal(result.summary, 'Copilot returned a prose review (structured JSON parsing failed).');
    assert.equal(result.verdict, 'comment');
    assert.equal(result.findings.length, 1);
